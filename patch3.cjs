...
let pEdu = `// Sort education chronologically / by level
...
c = c.replace(/for \(let i = 1; i <= 5; i\+\+\) \{\r?\n\s*let pE = Object\.assign\(\{\}, eduList\[i \- 1\] \|\| \{\}\);/, pEdu);

let pJob = `// Sort jobs chronologically
...
c = c.replace(/for \(let i = 1; i <= 3; i\+\+\) \{\r?\n\s*let pJ = Object\.assign\(\{\}, jobList\[i \- 1\] \|\| \{\}\);/, pJob);

let pFam = `// Sort family by age descending
...
c = c.replace(/for \(let i = 1; i <= 6; i\+\+\) \{\r?\n\s*let kF = Object\.assign\(\{\}, famList\[i \- 1\] \|\| \{\}\);/, pFam);

let pGajiJob = `let finalGaji = '\\u00A5&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-';
    if (gaji) {
...
c = c.replace(/if \(i > 2 && !\(pt_id \|\| msk \|\| klr\)\) continue;/, pGajiJob);

let pGajiFam = `let finalGaji = '\\u00A5&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-';
    if (g) {
...
c = c.replace(/if \(i > 3 && !\(hub \|\| nm \|\| u \|\| p\)\) continue;/, pGajiFam);

c = c.replace(/\$\{gaji \? '¥&nbsp;&nbsp;&nbsp;' \+ gaji : '¥&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-'\}/, '${finalGaji}');
c = c.replace(/\$\{g \? '¥&nbsp;&nbsp;&nbsp;' \+ g : '¥&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-'\}/, '${finalGaji}');
